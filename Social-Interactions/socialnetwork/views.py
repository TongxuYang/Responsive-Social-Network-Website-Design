...
@login_required
def view_the_whole(request):
    all_items = Item.objects.all().order_by('-time')
    return render(request, 'socialnetwork/globalstream.html', {'items': all_items})

@login_required
def home(request):
    # Sets up list of just the logged-in user's (request.user's) items
    items = Item.objects.filter(user=request.user)
    return render(request, 'socialnetwork/index.html', {'items' : items})

@login_required
def profile(request,username):
    user=User.objects.get(username=username)
    items=Item.objects.filter(user=user).order_by('-time')

    return render(request, 'socialnetwork/profile.html', {'items' :items } )


@login_required
def profile_edit (request,user):

  new_item = Profile_edit(text=request.POST.get('item', False), user=request.user)
  new_item.save()

  items = Profile_edit.objects.filter(user=request.user)
  return render(request, 'socialnetwork/editprofile.html',  {'items' :items })
